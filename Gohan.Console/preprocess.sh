...
} } } $lc++' 2> Gohan.Console/vcfs/_vcf.common.txt | sort -k1,1 -k2,2n > Gohan.Console/vcfs/_vcf.private.txt

echo Step 2 : converting common and private txt files to individual VCF files -
echo This also may take a while...

mkdir -p Gohan.Console/vcfs/split
time perl -lane 'BEGIN {
open IN, "Gohan.Console/vcfs/_vcf.common.txt" or die $!;
chomp(@common = <IN>); foreach (@common) {
 if (/^##/) { $headers .= "$_\n" } else { $headers .= $_; last }
} close IN }
if ($F[0] ne $previousSample) {
 close OUT if $previousSample;
 open OUT, ">Gohan.Console/vcfs/split/$F[0].vcf";
 print OUT "$headers\t$F[0]";
} $previousSample = $F[0];
print OUT "$common[$F[1]]\t$F[2]";
END { close OUT }' Gohan.Console/vcfs/_vcf.private.txt

echo Step 3 : compressing individual VCF files -
echo This also may take a while...

time for file in Gohan.Console/vcfs/split/*vcf; do
  gzip -f $file;
...
# Clean up
mv Gohan.Console/vcfs/split/*.vcf.gz Gohan.Console/vcfs/
rmdir Gohan.Console/vcfs/split/

rm Gohan.Console/vcfs/_vcf.private.txt
rm Gohan.Console/vcfs/_vcf.common.txt
